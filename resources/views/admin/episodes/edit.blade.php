        ...
        <div class="col-md-7">
            <div class="form-group">
                ...
                <select name="movie_id" class="form-control">
                    ...
                    <option value="{{ $movie->id }}" {{ $movie->id == $episode->movie->id ? 'selected' : '' }}>{{ $movie->title }}</option>
                    @endforeach
                </select>
            </div>
            <div class="form-group">
                <label for="">Title</label>
                <input type="text" name="title" value="{{ $episode->title }}" class="form-control" placeholder="Enter title">
            </div>
        </div>
        <div class="col-md-5">
            <div class="form-group">
                <label for="episode_number">Episode Number</label>
                <input type="number" name="episode_number" value="{{ $episode->episode_number }}" class="form-control" required>
            </div>
            <div class="form-group">
